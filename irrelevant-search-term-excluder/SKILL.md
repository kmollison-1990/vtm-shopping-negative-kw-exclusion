...
- If any Google Ads Agent or Slack Agent call fails with a "value too long for column" /
  P2000-style error, the delegation message was likely expanded beyond the ~900-character
  guideline in "Delegation Guidelines" above — trim it back to the literal template for that
  step rather than adding restated context.
